...
import { parseCalendar, applyEntryUpdate } from "../../../../../agents/lib/calendar";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  // the claim back to "queued" if the run fails before the draft lands on
  // its preview branch.
  ...
  try {
    ...
    // ----- Claim the entry on main BEFORE the expensive pipeline -----
    // This is the source-of-truth flip the old code never made on main: it
    // only flipped the entry on the preview branch, so main kept the entry
    // "queued" and every subsequent cron re-drafted + re-sent the same
    // article until it was approved/rejected. Flipping it here removes it
    // from the queued candidate set immediately.
    //
    // It also doubles as a concurrency lock. commitFilesToBranch advances
    // the ref with force:false (compare-and-swap), so if two cron runs fire
    // at once, both read the same head and only the first claim commit
    // lands; the loser's commit is rejected as a non-fast-forward and falls
    // into the catch. A staggered second run instead reads the already
    // "awaiting-approval" entry above and exits via the no-candidate path.
    //
    // The preview branch carries the same queued -> awaiting-approval flip
    // (runDraftPipeline computes it), so the eventual approval merge sees an
    // identical change on both sides and does not conflict.
    const claimedCalendarYaml = applyEntryUpdate({
      yaml: calRes.content,
      slug: entry.slug,
    ...
    await commitFilesToBranch({
      branch: "main",
      files: [{ path: CALENDAR_PATH, content: claimedCalendarYaml }],
      message: `Calendar: ${entry.slug} queued -> awaiting-approval (claimed by daily-draft cron)`,
    });
    claimedSlug = entry.slug;
    log(`[daily-draft] claimed ${entry.slug} on main (queued -> awaiting-approval).`);
    ...
    // The draft is safely on its preview branch now, so the entry is
    // genuinely "awaiting-approval" on main. Clear the rollback marker - a
    // later failure (e.g. Telegram send) must NOT revert it to queued, or
    // tomorrow's cron would draft it all over again.
    claimedSlug = null;
    ...
    // If we claimed an entry on main but never landed its draft, roll the
    // claim back to "queued" so a future run retries it instead of leaving
    // it stranded in "awaiting-approval" with no preview branch or message.
    // (When the claim commit itself lost a concurrency race, claimedSlug is
    // still null, so we correctly skip the rollback for that run.)
    if (claimedSlug) {
      try {
        const { content } = await getFileContent({ path: CALENDAR_PATH, ref: "main" });
        const reverted = applyEntryUpdate({
          yaml: content,
          slug: claimedSlug,
        ...
        await commitFilesToBranch({
          branch: "main",
          files: [{ path: CALENDAR_PATH, content: reverted }],
          message: `Calendar: ${claimedSlug} awaiting-approval -> queued (daily-draft failed, rolled back)`,
        });
        log(`[daily-draft] rolled ${claimedSlug} back to queued.`);
      } catch (rollbackErr) {
        const rmsg = rollbackErr instanceof Error ? rollbackErr.message : String(rollbackErr);
        log(`[daily-draft] rollback FAILED for ${claimedSlug}: ${rmsg}`);
      }
    }
